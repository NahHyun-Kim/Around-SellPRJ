...
public class SearchMapper implements ISearchMapper {
    ...
    public void insertKeyword(NoticeDTO pDTO) throws Exception {
        ...
        redisDB.expire(redisKey, 1, TimeUnit.DAYS); // 2일 후 삭제됨.
    ...
    public void insertGoods(NoticeDTO pDTO) throws Exception {
        ...
        tmp.put("goods_no", goods_no);
        tmp.put("imgs", imgs);
        tmp.put("goods_title", goods_title);

        String jsonGoods = tmp.toJSONString();
    ...
    public Set getGoods(NoticeDTO pDTO) throws Exception {
        ...
        if (redisDB.hasKey(redisKey)) {
            ...
            // 저장된 전체 레코드 수
            long cnt = redisDB.opsForZSet().size(redisKey);
            log.info("현재 레코드 수 (cnt) : " + cnt);

            // 전체값을 가져와 rSet에 담아 반환
    ...
    public void rmKeyword(NoticeDTO pDTO) throws Exception {
        ...
        // 저장된 key값에서 조회하여 가장 최근에 클릭한 상품을 삭제
        if (redisDB.hasKey(redisKey)) {
            ...
            try {
                redisDB.opsForZSet().removeRangeByScore(redisKey, cnt, cnt);

                cnt = redisDB.opsForZSet().size(redisKey);
                log.info("지우고 난 레코드 수 (cnt) : " + cnt);

            } catch(Exception e) {
                log.info("redis 삭제 오류! : " + e.toString());
                e.printStackTrace();
            }

        } else {
            log.info("기존 키 존재하지 않음!");
        }
        }
